...
- **Testing**: 562 tests total — 246 on the backend (schemas → `PriceFeed` → routes → WS broadcaster → price alerts → history → env var fail-fast behavior → auth routes/rate-limiting → alert-delivery user scoping → watchlist size cap, all wired into CI) and 316 on the frontend (hooks, API client, portfolio maths, WebSocket message validation, every component and screen, and an `App.tsx` integration suite covering the real wiring between them). See [Setup](#-setup) for how to run them.
...
- **Password reset**: single-use token, 1-hour expiry, cleared in the same write that sets the new password. Both endpoints answer identically regardless of whether the address or token exists, so neither can be used to enumerate accounts. **Known gap:** sessions are stateless (a signed cookie carrying only a user id), so resetting a password does *not* invalidate sessions already open elsewhere. Fixing that properly needs a per-user session epoch stored alongside the password hash and mixed into the cookie — worth doing before this app has users who'd care.
